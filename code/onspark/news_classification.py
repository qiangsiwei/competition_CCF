...
if __name__ == "__main__":
	...
	lines = sc.textFile('hdfs://namenode.omnilab.sjtu.edu.cn/user/qiangsiwei/competition_CCF/t_lable_group_comp_1.sort.seg.txt', 1)
	counts = lines.map(lambda x : extract(x)) \
			.filter(lambda x : x[0]!=-1) \
			.sortByKey() \
			.map(lambda x : x[1])
	output = counts.saveAsTextFile("./competition_CCF/tag_neg_t_lable_group_comp_1")
