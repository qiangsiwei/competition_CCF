...
if __name__ == "__main__":
	...
	for line in gzip.open("../../data/original/t_lable_group_comp_4.sort.seg.txt.gz"):
		...
		day = (int(time.mktime(time.strptime(line.strip().split("\t")[1][1:-1],'%Y-%m-%d %H:%M:%S')))-int(time.mktime(time.strptime("2013-01-01 00:00:00",'%Y-%m-%d %H:%M:%S'))))/(24*3600)
		if day >= 0:
			...
			if s4 >= 1 and s5 >= 1:
				cls = 3
			try:
				content_orig.decode("utf-8").encode("utf-8")
				" ".join([k+":"+str(v) for k,v in nr_map_t.iteritems()]).decode("utf-8").encode("utf-8")
				" ".join([k+":"+str(v) for k,v in ns_map_t.iteritems()]).decode("utf-8").encode("utf-8")
				if cls != 0:
					...
					file2.write(cid+"\t"+str(cls)+"\t"+str(day)+"\t"+"\""+" ".join([k+":"+str(v) for k,v in nr_map_t.iteritems()])+"\""+"\t"+"\""+" ".join([k+":"+str(v) for k,v in ns_map_t.iteritems()])+"\""+"\t"+content_orig+"\n")
			except:
				continue
	fileinput.close()
	file1.close()
	...
	event_map = {}
	c = 0
	for line in fileinput.input("../../data/classify/corpus_tfidf_event.txt"):
		part = line.strip().split("\t")
